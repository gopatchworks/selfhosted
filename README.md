...
run as `pre-install,pre-upgrade` hooks; seeders run as `pre-install` hooks. If
`app.key` is empty, the app chart generates a stable APP_KEY Secret automatically.
