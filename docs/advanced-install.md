...
This guide installs the split Helm charts manually on a local kind cluster.
For the guided installer, see the [main README](../README.md#getting-started).
For manual installation on an existing cluster or with GitOps, see the
[existing-cluster guide](getting-started.md).

These commands run from a repository checkout:

```bash
git clone https://github.com/gopatchworks/selfhosted.git
cd selfhosted
```
...
Have your Patchworks license key and Quay image pull credentials ready. See the
[existing-cluster guide](getting-started.md) for configuring the license, pull
secret, and initial company/admin in your shared values file.
...
```bash
kubectl -n patchworks get pods --watch
```
...
```bash
kubectl -n patchworks port-forward svc/patchworks-gateway 8080:80
```
...
kubectl -n patchworks logs -l app.kubernetes.io/component=gateway -f
kubectl -n patchworks logs -l app.kubernetes.io/component=workers -f

# Run an artisan command
kubectl -n patchworks exec -it deploy/patchworks-gateway -- php artisan <command>

# Open a MySQL shell
kubectl -n patchworks exec -it deploy/patchworks-mysql -- mysql -upatchworks -p core

# MinIO console. The password is generated in patchworks-s3-auth by default.
kubectl -n patchworks port-forward svc/patchworks-s3 9001:9001
...
Usually resource pressure. Increase the CPU and memory available to Docker in Docker Desktop → Settings → Resources.

```bash
kubectl describe node patchworks-control-plane
kubectl -n patchworks describe pod <stuck-pod>
```
...
kubectl -n patchworks delete pod -l app.kubernetes.io/component=elasticsearch
```
...
```bash
kubectl -n patchworks logs job/patchworks-core-migrations -c wait-for-deps
kubectl -n patchworks logs job/patchworks-core-migrations -c core-migrations
```
