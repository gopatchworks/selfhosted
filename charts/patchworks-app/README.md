...
| `s3.bucketCreationEndpoint` | `""` | Optional pwops-compatible endpoint for creating tenant-specific buckets; defaults to the resolved S3 endpoint |
...
leave `pusher.enabled=false` and set `pusher.external.host`, `port`, and
`scheme`.

| Key | Default | Description |
|-----|---------|-------------|
| `pusher.enabled` | `false` | Use in-cluster Soketi from the infra chart |
| `pusher.appId` | `""` | Pusher app ID. Generated by the infra chart when `pusher.enabled=true`, omitted, and `credentials.autoGenerate=true` |
| `pusher.appKey` | `""` | Pusher app key. Generated by the infra chart when `pusher.enabled=true`, omitted, and `credentials.autoGenerate=true` |
| `pusher.appSecret` | `""` | Pusher app secret. Generated by the infra chart when `pusher.enabled=true`, omitted, and `credentials.autoGenerate=true` |
