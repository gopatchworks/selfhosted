...
| Key | Default | Description |
|-----|---------|-------------|
| `app.key` | `""` | Laravel `APP_KEY`; the app chart generates a stable key when omitted |
| `app.previousKeys` | `[]` | Older Laravel keys passed through to the app chart as `APP_PREVIOUS_KEYS` |
...
| `app.existingSecret.previousKeysKey` | `""` | Optional key containing comma-separated `APP_PREVIOUS_KEYS` |
...
| `mysql.auth.database` | `core` | Database created on MySQL's first start and ensured by the setup hook when applicable |
| `mysql.databases` | `[]` | Additional schema names created/granted on install and upgrade; use 1–64 letters, digits, underscores or hyphens |
...
When `mysql.enabled=true` and either Fabric shares that MySQL instance or
`mysql.databases` is nonempty, the `mysql-setup` hook ensures
`mysql.auth.database`, each additional schema, and the shared Fabric schema
when applicable. This runs on install and upgrade, including an existing MySQL PVC.
The application user's existing broad privileges are unchanged. The list
creates schemas; it does not create isolated per-tenant users, registry records,
or migrate application tables. With external MySQL, provision schemas and
grants outside the chart.

For example, give Core, landlord and two tenants separate schemas on bundled
MySQL with this shared values file:
...
mysql:
  ...
  auth:
    database: core
  databases: [landlord, tenant_acme, tenant_globex]
...
Install or upgrade infrastructure before the app. Tenant registry schema names
must match the provisioned tenant schemas. The app still owns onboarding and
tenant migrations; changing these values does not move existing data.

---

## Landlord and tenant databases

These shared values configure app connections; the infra chart provisions only
the bundled schemas listed under `mysql.*`. The app chart maps each connection
to the different Core and Monocore variable names. For the mapping, registry
assignment rules and external-server example, see the
[app database reference](../patchworks-app/README.md#landlord-and-tenant-databases).

Empty landlord/default tenant host and username fields, zero ports, and an
empty landlord schema inherit the resolved `mysql.*` configuration. A
null/unset password inherits its password/Secret; an explicit password,
including `""`, clears that inherited Secret. An explicitly supplied
`existingSecret.name` takes precedence. Connection override passwords are not
generated. Secrets must exist in every consuming app or hook namespace.

| Key | Default | Description |
|-----|---------|-------------|
| `database.landlord.host` | `""` | Landlord host; empty inherits MySQL host |
| `database.landlord.port` | `0` | Landlord port; zero inherits MySQL port |
| `database.landlord.database` | `""` | Landlord schema; empty inherits MySQL database |
| `database.landlord.username` | `""` | Landlord username; empty inherits MySQL username |
| `database.landlord.password` | `null` | Landlord password; null/unset inherits, explicit `""` uses an empty password |
| `database.landlord.existingSecret.name` | `""` | Secret containing the landlord password |
| `database.landlord.existingSecret.passwordKey` | `password` | Password key in the landlord Secret |
| `database.landlord.readHost` | `""` | Optional Core read host; Monocore uses the write host |
| `database.tenant.host` | `""` | Default tenant host; empty inherits MySQL host |
| `database.tenant.port` | `0` | Default tenant port; zero inherits MySQL port |
| `database.tenant.username` | `""` | Default tenant username; empty inherits MySQL username |
| `database.tenant.password` | `null` | Default tenant password; null/unset inherits, explicit `""` uses an empty password |
| `database.tenant.existingSecret.name` | `""` | Secret containing the default tenant password |
| `database.tenant.existingSecret.passwordKey` | `password` | Password key in the default tenant Secret |
| `database.tenant.readHost` | `""` | Optional Core read host; Monocore uses the write host |
| `database.tenant.primaryServerId` | `""` | Optional Fabric database-server record ID for Core new tenant placement; distinct from its credential ID. Skips the default-endpoint seed database Job |
| `database.tenant.servers` | `{}` | Additional tenant servers keyed by application credential ID; IDs use `A-Za-z0-9_` |
| `database.tenant.servers.<id>.host` | required | Assigned server host; no inherited connection fields |
| `database.tenant.servers.<id>.port` | `3306` | Assigned server port |
| `database.tenant.servers.<id>.username` | required | Assigned server username |
| `database.tenant.servers.<id>.password` | `""` | Nonempty password required unless an existing Secret is supplied |
| `database.tenant.servers.<id>.existingSecret.name` | `""` | Secret containing the assigned server password |
| `database.tenant.servers.<id>.existingSecret.passwordKey` | `password` | Password key in the assigned server Secret |
| `database.tenant.servers.<id>.readHost` | `""` | Optional Core assigned-server read host; Monocore uses the write host |

Tenant schemas are chosen dynamically from application registry records, so
there is no `database.tenant.database` value. The chart does not insert those
records or assign tenants to server entries. Fabric's own database remains
configured through `fabric.mysql.*`.

The app's seed database Job uses only the default tenant endpoint and is
skipped when `database.tenant.primaryServerId` is set. Application provisioning
and tenant migrations must follow that registry assignment. For preprovisioned
schemas, set `seeds.tenant.createDatabase=false`.

Monocore receives both canonical `DB_TENANT_*` variables and `TENANT_DB_*`
compatibility aliases. Its image must implement independent default tenant
connections and assigned-server credentials; current local Monocore source
was checked, but the default `v0.1.43` image was not validated for these features.
